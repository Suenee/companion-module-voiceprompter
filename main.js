...
class VoicePrompterInstance extends InstanceBase {
  config={}; ws=null; reconnectTimer=null; heartbeatTimer=null; heartbeatTimeout=null; pendingPingId=null; pendingStatusBarModeId=null; destroyed=false
  heartbeatIntervalMs=DEFAULT_HEARTBEAT_MS; lastActivityAt=Date.now(); peerConnected=false; statusBarModeKnown=false
  ...
  defineVariables(){this.setVariableDefinitions([
    ...
    {variableId:'status_bar_mode',name:'Status Bar mode (off/top/bottom)'},{variableId:'status_bar_enabled',name:'Status Bar enabled (1/0)'},
  ...
  defineActions(){
    ...
    const zoneCountChoices=[1,2,3,4,5,6].map(n=>({id:n,label:String(n)}))
    const zoneOptions=[{type:'dropdown',id:'zoneCount',label:'Number of zones',choices:zoneCountChoices,default:1}]
    for(let i=1;i<=6;i++){
      zoneOptions.push({type:'textinput',id:`zone${i}Text`,label:`Zone ${i} text`,default:'',useVariables:true,multiline:false,isVisible:o=>Number(o.zoneCount??1)>=i})
      zoneOptions.push({type:'dropdown',id:`zone${i}Align`,label:`Zone ${i} alignment`,choices:alignChoices,default:i===1?'left':i===6?'right':'center',isVisible:o=>Number(o.zoneCount??1)>=i})
    }
    this.setActionDefinitions({
      ...
      status_bar:{name:'Set Status Bar',options:zoneOptions,callback:async action=>{const count=Number(action.options.zoneCount??1);if(!Number.isInteger(count)||count<1||count>6)throw new Error('Status Bar must contain 1 to 6 zones');const zones=[];for(let i=1;i<=count;i++){const text=await this.parseVariablesInString(String(action.options[`zone${i}Text`]??''));const align=String(action.options[`zone${i}Align`]??'left');if(!STATUS_BAR_ALIGNS.has(align))throw new Error(`Invalid alignment for zone ${i}`);zones.push({text,align})}this.sendVpp(this.envelope('call','vp',{method:'setStatusBar',args:{zones},expectsResponse:false}))}},
  ...
  handleMessage(rawData){const raw=Buffer.isBuffer(rawData)?rawData.toString('utf8'):String(rawData);let m;try{m=JSON.parse(raw)}catch{this.protocolFailure('Received invalid JSON',raw);return}if(!isObject(m)){this.protocolFailure('JSON root must be an object',raw);return}const envErr=this.validateEnvelope(m);if(envErr){this.protocolFailure(envErr,raw);return}
    ...
    if(this.pendingStatusBarModeId&&m.from==='vp'&&m.correlationId===this.pendingStatusBarModeId&&(m.type==='response'||m.type==='error')){
      this.pendingStatusBarModeId=null
      if(m.type==='response'){
        const result=isObject(m.result)?m.result:{}
        if(!hasOnlyKeys(result,['mode'])||!STATUS_BAR_MODES.has(result.mode)){this.protocolFailure('Invalid getStatusBarMode response',raw);return}
    ...
    if(m.from==='vp'&&!peerWasConnected&&!this.statusBarModeKnown)this.requestStatusBarMode()
