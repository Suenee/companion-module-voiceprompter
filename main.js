...
class VoicePrompterInstance extends InstanceBase {
 ...
 async announceDisconnecting(reason='user'){
  ...
  const msg=this.envelope('event','vp',{event:'disconnecting',args:{reason},expectsResponse:false})
 ...
 defineVariables(){this.setVariableDefinitions([{variableId:'connected',name:'VPBridge connected (1/0)'},{variableId:'vp_connected',name:'VoicePrompter mailbox connected (1/0)'},{variableId:'connection_state',name:'Connection state'},{variableId:'status_bar_mode',name:'Status Bar mode (off/top/bottom)'},{variableId:'status_bar_enabled',name:'Status Bar enabled (1/0)'},{variableId:'heartbeat_interval_ms',name:'Heartbeat interval ms'},{variableId:'protocol_version',name:'Protocol version'},{variableId:'id',name:'Message ID'},{variableId:'correlation_id',name:'Correlation ID'},{variableId:'type',name:'Message type'},{variableId:'from',name:'From mailbox'},{variableId:'recipient',name:'Recipient mailbox'},{variableId:'method',name:'Call method'},{variableId:'event',name:'Event name'},{variableId:'command',name:'Marker command'},{variableId:'marker_args',name:'Marker arguments JSON'},{variableId:'arg_offset',name:'Call args.offset'},{variableId:'data',name:'Data JSON'},{variableId:'result',name:'Result JSON'},{variableId:'error_code',name:'Error code'},{variableId:'error_message',name:'Error message'},{variableId:'error_details',name:'Error details JSON'},{variableId:'source_app',name:'Source application'},{variableId:'source_version',name:'Source application version'},{variableId:'source_instance',name:'Source instance'},{variableId:'timestamp',name:'Protocol timestamp'},{variableId:'payload',name:'Complete JSON payload'},{variableId:'last_received',name:'Local receive timestamp'},{variableId:'last_sent',name:'Local send timestamp'},{variableId:'protocol_error',name:'Last VPP validation error'}])}
 resetVariables(){this.setVariableValues({connected:0,vp_connected:0,connection_state:'disconnected',status_bar_mode:'',status_bar_enabled:0,heartbeat_interval_ms:DEFAULT_HEARTBEAT_MS,protocol_version:'',id:'',correlation_id:'',type:'',from:'',recipient:'',method:'',event:'',command:'',marker_args:'',arg_offset:'',data:'',result:'',error_code:'',error_message:'',error_details:'',source_app:'',source_version:'',source_instance:'',timestamp:'',payload:'',last_received:'',last_sent:'',protocol_error:''})}
 setStatusBarModeVariables(mode){if(!STATUS_BAR_MODES.has(mode))return false;this.statusBarModeKnown=true;this.setVariableValues({status_bar_mode:mode,status_bar_enabled:mode==='off'?0:1});return true}
 clearStatusBarModeVariables(){this.statusBarModeKnown=false;this.pendingStatusBarModeId=null;this.setVariableValues({status_bar_mode:'',status_bar_enabled:0})}
 validateZoneText(text){text=String(text);if(unicodeLength(text)>STATUS_BAR_TEXT_MAX)throw new Error(`Status Bar zone text exceeds ${STATUS_BAR_TEXT_MAX} characters`);return text}
 async resolveZoneCount(raw){const resolved=await this.parseVariablesInString(String(raw??'')),count=Number(String(resolved).trim());return Number.isInteger(count)&&count>=1&&count<=6?count:null}
 defineActions(){
  const navChoices=[{id:'GO_START',label:'Go Start'},{id:'MARKER_BACK',label:'Marker Back'},{id:'GO_BACK',label:'Go Back'},{id:'GO_CURRENT',label:'Go Current'},{id:'GO_NEXT',label:'Go Next'},{id:'MARKER_NEXT',label:'Marker Next'},{id:'GO_FINISH',label:'Go Finish'}]
  const modeChoices=[{id:'off',label:'Off'},{id:'top',label:'Top'},{id:'bottom',label:'Bottom'}]
  const alignChoices=[{id:'left',label:'Left'},{id:'center',label:'Center'},{id:'right',label:'Right'}]
  const zoneCountChoices=['1','2','3','4','5','6'].map(n=>({id:n,label:n}))
  const zoneOptions=[{type:'dropdown',id:'zoneCount',label:'Number of active zones',choices:zoneCountChoices,default:'1'}]
  for(let i=1;i<=6;i++){
    zoneOptions.push({type:'textinput',id:`zone${i}Text`,label:`ZONE ${i} - Content`,default:'',useVariables:true,multiline:false})
    zoneOptions.push({type:'dropdown',id:`zone${i}Align`,label:`ZONE ${i} - Alignment`,choices:alignChoices,default:i===1?'left':i===6?'right':'center'})
  }
  this.setActionDefinitions({
    navigation:{name:'Navigation',options:[{type:'dropdown',id:'instruction',label:'Instruction',choices:navChoices,default:'GO_NEXT'},{type:'textinput',id:'offset',label:'Offset',default:'1',useVariables:true,multiline:true,isVisible:o=>NAV_METHODS[String(o.instruction??'GO_NEXT')]?.[1]===true}],callback:async action=>{const entry=NAV_METHODS[String(action.options.instruction??'GO_NEXT')];if(!entry)throw new Error('Unsupported Navigation instruction');const[method,usesOffset]=entry,args={};if(usesOffset){const raw=await this.parseVariablesInString(String(action.options.offset??'')),offset=Number(String(raw).trim());if(!Number.isInteger(offset))throw new Error('Navigation offset must resolve to an integer');args.offset=offset}this.sendVpp(this.envelope('call','vp',{method,args,expectsResponse:true}))}},
    status_bar_mode:{name:'Status Bar Mode',options:[{type:'dropdown',id:'mode',label:'Mode',choices:modeChoices,default:'bottom'}],callback:async action=>{const mode=String(action.options.mode??'');if(!STATUS_BAR_MODES.has(mode))throw new Error('Invalid Status Bar mode');this.sendVpp(this.envelope('call','vp',{method:'setStatusBarMode',args:{mode},expectsResponse:false}))}},
    status_bar_zone_count:{name:'Set Status Bar Zone Count',options:[{type:'textinput',id:'count',label:'Number of active zones (1-6)',default:'1',useVariables:true,multiline:false}],callback:async action=>{const count=await this.resolveZoneCount(action.options.count);if(count===null)return;this.sendVpp(this.envelope('call','vp',{method:'setStatusBarZoneCount',args:{count},expectsResponse:false}))}},
    status_bar:{name:'Set Status Bar',options:zoneOptions,callback:async action=>{const count=Number(String(action.options.zoneCount??'1'));if(!Number.isInteger(count)||count<1||count>6)return;this.sendVpp(this.envelope('call','vp',{method:'setStatusBarZoneCount',args:{count},expectsResponse:false}));for(let i=1;i<=count;i++){const text=this.validateZoneText(await this.parseVariablesInString(String(action.options[`zone${i}Text`]??''))),align=String(action.options[`zone${i}Align`]??'left');if(!STATUS_BAR_ALIGNS.has(align))throw new Error(`Invalid alignment for zone ${i}`);this.sendVpp(this.envelope('call','vp',{method:'setStatusBarZone',args:{index:i,text,align},expectsResponse:false}))}}},
    status_bar_zone:{name:'Set Status Bar Zone',options:[{type:'textinput',id:'index',label:'Zone number (1-6)',default:'1',useVariables:true,multiline:false},{type:'textinput',id:'text',label:'Content',default:'',useVariables:true,multiline:false},{type:'dropdown',id:'align',label:'Alignment',choices:alignChoices,default:'left'}],callback:async action=>{const raw=await this.parseVariablesInString(String(action.options.index??'')),index=Number(String(raw).trim());if(!Number.isInteger(index)||index<1||index>6)return;const text=this.validateZoneText(await this.parseVariablesInString(String(action.options.text??''))),align=String(action.options.align??'left');if(!STATUS_BAR_ALIGNS.has(align))throw new Error('Invalid Status Bar zone alignment');this.sendVpp(this.envelope('call','vp',{method:'setStatusBarZone',args:{index,text,align},expectsResponse:false}))}},
    clear_status_bar:{name:'Clear Status Bar',options:[],callback:async()=>{this.sendVpp(this.envelope('call','vp',{method:'clearStatusBar',args:{},expectsResponse:false}))}},
 ...
 handleMessage(rawData){
  const raw=Buffer.isBuffer(rawData)?rawData.toString('utf8'):String(rawData);let m
  ...
  const envErr=this.validateEnvelope(m);if(envErr){this.protocolFailure(envErr,raw);return}
  const isPeerDisconnecting=m.type==='event'&&m.event==='disconnecting'&&m.from==='vp'
  const peerWasConnected=this.peerConnected
  if(m.from==='vp'&&!isPeerDisconnecting){this.lastActivityAt=Date.now();this.peerConnected=true;this.updateConnectionStatus()}
  ...
  if(this.pendingStatusBarModeId&&m.from==='vp'&&m.correlationId===this.pendingStatusBarModeId&&(m.type==='response'||m.type==='error')){this.pendingStatusBarModeId=null;if(m.type==='response'){const result=isObject(m.result)?m.result:{};if(!hasOnlyKeys(result,['mode'])||!STATUS_BAR_MODES.has(result.mode)){this.protocolFailure('Invalid getStatusBarMode response',raw);return}this.setStatusBarModeVariables(result.mode)}else this.clearStatusBarModeVariables()}
  ...
  if(m.type==='event'){
   if(m.event==='marker'){
    ...
    if(!isObject(m.args)||!hasOnlyKeys(m.args,['mode'])||!STATUS_BAR_MODES.has(m.args.mode)){if(m.expectsResponse===true)this.sendError(m,'INVALID_MESSAGE','Invalid statusBarModeChanged event');this.protocolFailure('Invalid statusBarModeChanged event',raw);return}
    this.setStatusBarModeVariables(m.args.mode)
   }else if(m.event==='disconnecting'){
    if(!isObject(m.args)||!hasOnlyKeys(m.args,['reason'])||typeof m.args.reason!=='string'){if(m.expectsResponse===true)this.sendError(m,'INVALID_MESSAGE','Invalid disconnecting event');this.protocolFailure('Invalid disconnecting event',raw);return}
  ...
  if(m.type==='call'){if(typeof m.method!=='string'||!isObject(m.args)){if(m.expectsResponse===true)this.sendError(m,'INVALID_MESSAGE','Invalid call');this.protocolFailure('Invalid call',raw);return}if(m.expectsResponse===true)this.sendError(m,'UNKNOWN_METHOD',`VPM has no public method "${m.method}"`)}
  const source=isObject(m.source)?m.source:{},error=isObject(m.error)?m.error:{},callArgs=m.type==='call'&&isObject(m.args)?m.args:{}
  ...
  if(m.from==='vp'&&!isPeerDisconnecting&&!peerWasConnected&&!this.statusBarModeKnown)this.requestStatusBarMode()
